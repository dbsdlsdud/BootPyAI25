...
# 파일 입출력을 위한 모듈
import base64
# 데이터를 Base64로 인코딩 디코딩 https://ko.wikipedia.org/wiki/%EB%B2%A0%EC%9D%B4%EC%8A%A464
...
# YOLOv8 모델 로드 (yolov8n.pt 모델의 가중치 파일)
print('???????????',model.names)
...
class DetectionResult(BaseModel): # pydantic을 사용하여 데이터 모델을 정의(응답 데이터를 구조화)
    ...
    image : str # Basw64로 인코딩된 탐지 결과 이미지
...
def detect_objects(image: Image):
    ...
    return result_image
    # 결론 : YOLO모델로 객체 탐지 수행
    # 탐지된 객체에 바운딩 박스를 그리고 정확도 점수를 이미지에 표시
    # 결과 이미지를 PIL 이미지로 변환하여 반환
        
@app.get("/") # get 방식의 요청 테스트용 메시지를 json 형식으로 반환
async def read_root():
