...
# 미들웨어는 모든 요청에 대해 실행되며 요청을 처리하기 전(응답을 반환하기 전)에 특정작업을 수행할 수 있다.
# 예를들어 로깅, 인증, cors처리, 압축 등등을 처리하는 용 -> 특정작업
import logging # 로그 출력용

app = FastAPI( # 생성자를 통해 postman을 대체하는 문서화 툴이 내장되어 있음 
...
class Item(BaseModel): # 아이템 객체 생성(BaseModel : 객체 연결 -> 상속)
    ...
    description : str = None # 필드2 : 상품설명(문자열/null)
    price : float # 필드3 : 가격(실수형)
    tax : float = None # 필드4 : 세금(실수형/null)

# 컨트롤러 검증은 postman으로 활용해 보았는데 내장된 백검증툴도 있다.
...
async def create_item(item: Item):
    # BaseModel은 데이터 모델링을 쉽게 도와주고, 유효성 검사를 수행한다.
    # 잘못된 데이터가 들어오면 422 오류코드를 반환한다.
    return item

@app.get("/items/{item_id}") # http://ip주소:포트번호/item/1
